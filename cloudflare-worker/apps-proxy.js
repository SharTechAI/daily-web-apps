...
<main>
  ...
  <div class="cta-row">
    ...
    <a href="https://github.com/SharTechAI/daily-web-apps" class="btn-secondary" target="_blank" rel="noopener">
...
<script>
  fetch('/apps')
    .then(r => r.text())
    .then(html => {
      const matches = html.match(/class="app-card"/g);
      const count = matches ? matches.length : null;
      if (count) document.getElementById('app-count').textContent = count + '+';
    })
    .catch(() => {});
</script>
</body>
